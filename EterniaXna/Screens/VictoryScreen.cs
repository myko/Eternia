...
namespace EterniaXna.Screens
{
    public class VictoryScreen: MenuScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            rewardsListBox = AddListBox<Item>(Controls, new Vector2(Width / 2 - 200, 250), 400, 300);
            rewardsListBox.ZIndex = 0.2f;
            rewardsListBox.EnableCheckBoxes = true;
            for (int i = 0; i < 5; i++)
            {
                var item = generator.Generate(encounterDefinition.ItemLevel);
                rewardsListBox.Items.Add(item, new ItemTooltip(item) { Font = ContentManager.Load<SpriteFont>(@"Fonts\KootenaySmall") });
            }
            Controls.Add(rewardsListBox);

            var okButton = CreateButton("OK", new Vector2(Width / 2 - 75, Height - 300));
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            SpriteBatch.DrawString(Font, "You won!", new Vector2(Width / 2 - 350, 100), Color.White, 0.1f);
            ...
            if (events.Any())
            {
                ...
                foreach (var actorEvents in events.GroupBy(x => x.Actor))
                {
                    ...
                    y += 16;
                    SpriteBatch.DrawString(rewardsListBox.Font, actorEvents.Key.Name, new Vector2(200, y), Color.White, 0.1f);
                    SpriteBatch.DrawString(rewardsListBox.Font, (actorEvents.Sum(x => x.Damage) / duration.TotalSeconds).ToString("0") + " DPS", new Vector2(350, y), Color.White, 0.1f);
                    SpriteBatch.DrawString(rewardsListBox.Font, meleeSwings.Count().ToString() + " swings", new Vector2(450, y), Color.White, 0.1f);
                    if (meleeSwings.Count() > 0)
                    {
                        SpriteBatch.DrawString(rewardsListBox.Font, (100 * missSwings.Count() / meleeSwings.Count()).ToString() + "% m", new Vector2(550, y), Color.White, 0.1f);
                        SpriteBatch.DrawString(rewardsListBox.Font, (100 * dodgeSwings.Count() / meleeSwings.Count()).ToString() + "% d", new Vector2(600, y), Color.White, 0.1f);
                        SpriteBatch.DrawString(rewardsListBox.Font, (100 * hitSwings.Count() / meleeSwings.Count()).ToString() + "% h", new Vector2(650, y), Color.White, 0.1f);
                        if (hitSwings.Count() > 0)
                            SpriteBatch.DrawString(rewardsListBox.Font, (100 * critSwings.Count() / hitSwings.Count()).ToString() + "% c", new Vector2(700, y), Color.White, 0.1f);
                    }
                }
            }
        }
